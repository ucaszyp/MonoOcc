...
class CVT(nn.Module):
    ...
    def forward(self, x): # B, N, C, H, W
        B, N, C, H, W = x.shape
        if W % self.downsample_ratio != 0 and H % self.downsample_ratio == 0:
            x = self.sep_conv(x.view(-1,C,H,W)).view(B,N,C,H//self.downsample_ratio,W//self.downsample_ratio+1)
        elif H % self.downsample_ratio != 0 and W % self.downsample_ratio == 0:
            x = self.sep_conv(x.view(-1,C,H,W)).view(B,N,C,H//self.downsample_ratio+1,W//self.downsample_ratio)
        elif H % self.downsample_ratio != 0 and W % self.downsample_ratio != 0:
            x = self.sep_conv(x.view(-1,C,H,W)).view(B,N,C,H//self.downsample_ratio+1,W//self.downsample_ratio+1)
        else:
        ...
        for i in range(self.iter_num):
            ...
            if W % self.downsample_ratio != 0 and H % self.downsample_ratio == 0:
                x = self.self_block[i](x,x_pos).reshape(B,N,C,H//self.downsample_ratio, W//self.downsample_ratio+1)
            elif H % self.downsample_ratio != 0 and W % self.downsample_ratio == 0:
                x = self.self_block[i](x,x_pos).reshape(B,N,C,H//self.downsample_ratio+1, W//self.downsample_ratio)
            elif H % self.downsample_ratio != 0 and W % self.downsample_ratio != 0:
                x = self.self_block[i](x,x_pos).reshape(B,N,C,H//self.downsample_ratio+1, W//self.downsample_ratio+1)
            else:
                x = self.self_block[i](x,x_pos).reshape(B,N,C,H//self.downsample_ratio, W//self.downsample_ratio)
            # x = self.self_block[i](x,x_pos).reshape(B,N,C,H,W)  
        if W % self.downsample_ratio != 0 and H % self.downsample_ratio == 0:
            x = self.sep_deconv(x.view(-1, C, H//self.downsample_ratio, W//self.downsample_ratio+1))
        elif H % self.downsample_ratio != 0 and W % self.downsample_ratio == 0:
            x = self.sep_deconv(x.view(-1, C, H//self.downsample_ratio+1, W//self.downsample_ratio))
        elif H % self.downsample_ratio != 0 and W % self.downsample_ratio != 0:
            x = self.sep_deconv(x.view(-1, C, H//self.downsample_ratio+1, W//self.downsample_ratio+1))
        else:
            x = self.sep_deconv(x.view(-1, C, H//self.downsample_ratio, W//self.downsample_ratio))
        
        
        if W % self.downsample_ratio != 0 and H % self.downsample_ratio == 0:
            x = x.view(B,N,C,H,W+1)
            x = x[:,:,:,:,:-1]
        elif H % self.downsample_ratio != 0 and W % self.downsample_ratio == 0:
            x = x.view(B,N,C,H+1,W)
            x = x[:,:,:,:-1,:]  
        elif H % self.downsample_ratio != 0 and W % self.downsample_ratio != 0:
            x = x.view(B,N,C,H+1,W+1)
            x = x[:,:,:,:-1,:-1]                 
        else:
            x = x.view(B,N,C,H,W)
